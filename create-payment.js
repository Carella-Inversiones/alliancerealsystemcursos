...
exports.handler = async (event) => {
  ...
  try {
    ...
    // A partir de 6 cursos (sea el pack de 6 o el pack completo de 12) el
    // precio ya no es la suma de precios individuales, así que armamos un
    // único ítem combinado con el total real calculado en el servidor.
    let itemsMP;
    if (esPackCompleto) {
      itemsMP = [{
    ...
    } else if (slugsValidos.length >= 6) {
      itemsMP = [{
        title: `Pack de ${slugsValidos.length} cursos — Alliance Real System`,
        quantity: 1,
        currency_id: "ARS",
        unit_price: total
      }];
    } else {
      itemsMP = slugsValidos.map(slug => ({
